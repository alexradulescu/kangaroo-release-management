...
import { Accordion, Button, Group, Paper, Text, Tooltip } from '@mantine/core'
...
export const MessageAccordion = memo(function MessageAccordion({
  ...
  const renderControls = () => (
    <Button.Group>
      <Button variant='light' color='blue' size='xs' onClick={handleCopyAndSlack} disabled={disabled}>
        Copy & Slack
      </Button>
      <Tooltip label='Copy'>
        <Button variant='light' color='gray' onClick={handleCopy} disabled={disabled} size='xs'>
          <Copy size={16} />
        </Button>
      </Tooltip>
      <Tooltip label='Open Slack'>
        <Button variant='light' color='gray' onClick={openSlack} disabled={disabled} size='xs'>
